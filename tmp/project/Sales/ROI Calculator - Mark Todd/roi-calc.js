...
(function () {
  ...
  var SECONDS_PER_YEAR = 365 * 24 * 3600; // 31,536,000 — drives the per-second cost of delay
  ...
  var state = {
    ...
    client: "Acme Corp (test client)", preparedBy: "Mark Todd",
    delayStart: null,
    sentAt: null,
    sentLink: ""
  };
  ...
  function mergeState(s) {
    ["country", "spend", "blended", "industry", "headcount", "peopleMode", "locations", "peoplePerLoc", "fundingMode", "supplierPct", "client", "preparedBy", "delayStart", "sentAt", "sentLink"].forEach(function (k) {
  ...
  // =================================================================
  //  COST-OF-DELAY CLOCK  (Mark Todd: the meter that keeps running)
  //  The net annual ROI, spread across the year, becomes a live
  //  per-second cost of indecision. Once the date the buyer started
  //  deliberating internally is set, the meter counts up in real time
  //  — on the hero, in the nav bar, and full screen — so the prospect
  //  OWNS it, SEES it, and FEELS every second of the pause.
  // =================================================================
  var clockRate = { perSecond: 0, perMinute: 0, perHour: 0, perDay: 0, net: 0 };
  var clockRAF = null;

  function clockIsRunning() { return !!state.delayStart && clockRate.perSecond > 0; }
  function clockElapsedSec() {
    if (!state.delayStart) return 0;
    return Math.max(0, (Date.now() - state.delayStart) / 1000);
  }
  function clockAccrued() { return clockRate.perSecond * clockElapsedSec(); }

  function pad2(n) { return (n < 10 ? "0" : "") + n; }
  function isoDay(ts) { var d = new Date(ts); return d.getFullYear() + "-" + pad2(d.getMonth() + 1) + "-" + pad2(d.getDate()); }
  function longDate(ts) {
    return new Date(ts).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
  }

  // split an amount into a grouped integer string + ".cc" cents
  function meterParts(n) {
    if (!isFinite(n) || n < 0) n = 0;
    var intPart = Math.floor(n);
    var cents = Math.floor((n - intPart) * 100);
    return { int: groupNum(intPart), dec: "." + pad2(cents) };
  }

  // paint the three live meters (hero, nav ticker, full screen)
  function paintMeters(n) {
    var sym = country().symbol, p = meterParts(n);
    var ci = $("#clockInt"), cd = $("#clockDec"), cc = $("#clockCur");
    if (ci) ci.textContent = p.int;
    if (cd) cd.textContent = p.dec;
    if (cc) cc.textContent = sym;
    var nv = $("#navTickerVal");
    if (nv) nv.textContent = sym + p.int + p.dec;
    var cash = $("#cashClock");
    if (cash && !cash.hidden) {
      var ccI = $("#ccInt"), ccD = $("#ccDec"), ccC = $("#ccCur");
      if (ccI) ccI.textContent = p.int;
      if (ccD) ccD.textContent = p.dec;
      if (ccC) ccC.textContent = sym;
    }
  }

  // rate grid markup: per second / minute / hour / day
  function ratesGrid() {
    var cells = [
      ["Per second", money(clockRate.perSecond, 2)],
      ["Per minute", money(clockRate.perMinute, 2)],
      ["Per hour", money(clockRate.perHour)],
      ["Per day", money(clockRate.perDay)]
    ];
    return cells.map(function (c) {
      return '<div class="roi-rate"><span class="rv">' + c[1] + '</span><span class="rl">' + c[0] + '</span></div>';
    }).join("");
  }

  function whoLabel() {
    return state.client && state.client.trim() ? state.client.trim() : "your organization";
  }

  // called from render() — recomputes the rate and refreshes clock chrome
  function renderClock(r) {
    var net = Math.max(0, r.net);
    clockRate.net = net;
    clockRate.perSecond = net / SECONDS_PER_YEAR;
    clockRate.perMinute = clockRate.perSecond * 60;
    clockRate.perHour = clockRate.perSecond * 3600;
    clockRate.perDay = net / 365;

    var started = !!state.delayStart;
    var startEl = $("#clockStart"), runEl = $("#clockRunning");
    if (startEl) startEl.hidden = started;
    if (runEl) runEl.hidden = !started;

    var grid = ratesGrid();
    var cr = $("#clockRates"); if (cr) cr.innerHTML = grid;
    var ccr = $("#ccRates"); if (ccr) ccr.innerHTML = grid;

    var sym = country().symbol;
    var cc = $("#clockCur"); if (cc) cc.textContent = sym;
    var ccc = $("#ccCur"); if (ccc) ccc.textContent = sym;

    var ccFor = $("#ccFor");
    if (ccFor) ccFor.textContent = "What indecision has cost " + whoLabel() + " so far";

    var eye = $("#clockEyebrow"), st = $("#clockStatus");
    if (net <= 0) {
      if (eye) eye.textContent = "Cost of delay · paused";
      if (st) st.textContent = "Tune the model below until the net return is positive — then every second of delay carries a price worth showing.";
      paintMeters(0);
    } else if (!started) {
      if (eye) eye.textContent = "Cost of delay · ready";
      if (st) st.innerHTML = "Set the date this first came up internally, then start the clock. From that moment, every second without a decision is about <b>" + money(clockRate.perSecond, 2) + "</b> gone.";
      paintMeters(0);
    } else {
      var days = Math.floor(clockElapsedSec() / 86400);
      if (eye) eye.textContent = "Cost of delay · live";
      if (st) st.innerHTML = "Since <b>" + longDate(state.delayStart) + "</b>, while this has been talked about internally without a decision, indecision has cost " +
        whoLabel() + " the figure above — " + days + (days === 1 ? " day" : " days") +
        " in, about <b>" + money(clockRate.perDay) + "</b> a day and climbing every second.";
    }

    var nav = $("#navTicker");
    if (nav) nav.hidden = !clockIsRunning();

    if (clockIsRunning()) paintMeters(clockAccrued());
  }

  // a single requestAnimationFrame loop drives the visible ticking
  ...
  // ---- full-screen cash clock ------------------------------------
  function openCashClock() {
    var cash = $("#cashClock");
    if (!cash) return;
    $("#ccFor").textContent = "What indecision has cost " + whoLabel() + " so far";
    $("#ccRates").innerHTML = ratesGrid();
    var st = $("#ccStatus");
    if (st) {
      st.textContent = clockIsRunning()
        ? "Counting since " + longDate(state.delayStart) + " · about " + money(clockRate.perDay) + " a day"
        : "Start the clock on the page to set the meter running.";
    }
    paintMeters(clockAccrued());
    cash.hidden = false;
    cash.offsetHeight; // reflow so the open transition plays
    cash.classList.add("is-open");
    document.body.style.overflow = "hidden";
  }
  function closeCashClock() {
    var cash = $("#cashClock");
    if (!cash) return;
    cash.classList.remove("is-open");
    document.body.style.overflow = "";
    setTimeout(function () { cash.hidden = true; }, 220);
  }

  function initClock() {
    var di = $("#delayDate");
    if (di) {
      di.max = isoDay(Date.now());
      if (state.delayStart) {
        di.value = isoDay(state.delayStart);
      } else {
        var d = new Date(); d.setDate(d.getDate() - 14); // sensible demo default
        di.value = isoDay(d.getTime());
      }
    }
    var startBtn = $("#startClockBtn");
    if (startBtn) startBtn.addEventListener("click", function () {
      var v = (di && di.value) ? di.value : isoDay(Date.now());
      var ts = new Date(v + "T09:00:00").getTime();
      if (!isFinite(ts) || ts > Date.now()) ts = Date.now();
      state.delayStart = ts;
      saveLocal(); render();
      toast("Delay clock started · the meter is running");
    });
    var resetBtn = $("#clockReset");
    if (resetBtn) resetBtn.addEventListener("click", function () {
      state.delayStart = null; saveLocal(); render(); toast("Delay clock reset");
    });
    ["#clockMeter", "#clockFull", "#navTicker"].forEach(function (sel) {
      var el = $(sel); if (el) el.addEventListener("click", openCashClock);
    });
    var ccClose = $("#cashClockClose");
    if (ccClose) ccClose.addEventListener("click", closeCashClock);
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape") { var cash = $("#cashClock"); if (cash && !cash.hidden) closeCashClock(); }
    });
    var copyBtn = $("#clockCopyLink");
    if (copyBtn) copyBtn.addEventListener("click", function () {
      copyToClipboard(buildShareUrl(), function () { toast("Live link copied — the clock keeps running for them too"); });
    });
  }
